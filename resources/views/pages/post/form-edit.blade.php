...
@section('content')
    ...
    @endforeach
                            ...
                            <div style="{{ ($post->postable !== null && get_class($post->postable) == \App\Models\Recruitment::class) || $post->postable_type == \App\Models\Recruitment::class ? '' : 'display:none' }}"
                                id="recruitment" class="row">
                                <div class="form-group mb-10 col-xl-12 col-12">
                                    <label for="" class="form-label">Doanh nghiệp</label>
                                    <select id="enterprise_edit_id" name="enterprise_id" class="form-select form-major" data-control="select2"
                                        data-placeholder="Chọn doanh nghiệp">
                                        <option value="0">Chọn doanh nghiệp</option>
                                        @foreach ($enterprises as $enterprise)
                                            <option @selected(old('enterprise_id') ? old('enterprise_id') == $enterprise->id : $post->enterprise_id == $enterprise->id) value="{{ $enterprise->id }}">
                                                {{ $enterprise->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group mb-10 col-xl-3 col-12">
                                    ...
                                    <select name="major_id" class="form-select form-major" data-control="select2"
                                        ...
                                        @foreach ($majors as $major)
                                            <option @selected(old('major') ? old('major') == $major->id : $post->major_id == $major->id) value="{{ $major->id }}">
                                                {{ $major->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group mb-10 col-xl-3 col-6">
                                    ...
                                    <select name="tax_number" id="tax_number" class="form-select form-major"
                                        data-control="select2" data-placeholder="Mã số thuế">
                                        <option value="0">Mã số thuế</option>
                                        @foreach ($tax_numbers as $tax_number)
                                            <option @selected(old('tax_number') ? old('tax_number') == $tax_number->tax_number : $post->tax_number == $tax_number->tax_number) value="{{ $tax_number->tax_number }}">
                                ...
                                <div class="form-group mb-10 col-xl-3 col-12">
                                    ...
                                    <select name="career_type" id="" class="form-select form-major"
                                        ...
                                        @foreach (config('util.CAREER_TYPES') as $key => $value)
                                            <option @selected(old('career_type') == $key || $post->career_type == $key) value="{{ $key }}">
                                                {{ $value }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group mb-10 col-xl-12 col-12">
                                    ...
                                    <textarea name="note" class="form-control" id="" cols="30" rows="3">{{ old('note') ?? $post->note }}</textarea>
                                </div>
                                <div class="form-group mb-10">
                    ...
                    <div class="row">
                        <div class="col-8">
                            ...
                            <div class="form-group mb-10">
                                ...
                                <textarea class="form-control" name="description" id="kt_docs_ckeditor_classic_edit" rows="3">{{ $post->description }}</textarea>
                                @error('description')
                                    <p class="text-danger">{{ $message }}</p>
                                @enderror
                            </div>
